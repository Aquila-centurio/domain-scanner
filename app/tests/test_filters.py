...
from tasks import load_and_filter  # noqa: E402
...
def test_zero_price_means_no_limit():
    """max_price=0 означает без ограничения по цене — любая цена проходит."""
    rows = [make_row("any.com", price=9999)]
    assert len(run_filter(rows, max_price=0)) == 1


def test_sorted_by_hours_left():
    """Результаты отсортированы по hours_left — сначала те что скоро заканчиваются."""
    rows = [
